...
context "/assets/index.html.erb" do
  ...
  before(:each) do
    ...
    @controller.template.should_receive(:user_user_asset_path).with(@user, asset_98).exactly(2).times.and_return('ASSET_98_PATH')
    @controller.template.should_receive(:user_user_asset_path).with(@user, asset_99).exactly(2).times.and_return('ASSET_99_PATH')
    @controller.template.should_receive(:edit_user_user_asset_path).with(@user, asset_98).and_return('EDIT_ASSET_98_PATH')
    @controller.template.should_receive(:edit_user_user_asset_path).with(@user, asset_99).times.and_return('EDIT_ASSET_99_PATH')
    @controller.template.should_receive(:new_user_user_asset_path).with(@user).times.and_return('NEW_ASSET_PATH')
